...
4. Worth an occasional pass: a validation error keeps the path (the error chip floats absolutely in the status bar — nothing below it moves); «Отмена»/Escape rolls the move back; a plain click on a filled cell in the word phase does nothing — except on the added-letter cell: the click reopens the keyboard, a picked letter replaces the pending one (the path is kept), and any cell tap / Escape closes the panel; the word itself is drawn by dragging only. At a phone viewport (≤560px) the keyboard spans the full screen width, pinned to the viewport edges.
